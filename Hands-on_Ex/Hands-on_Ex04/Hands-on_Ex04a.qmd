...
If we want to have the area under a ridgeline not filled with a single solid color but rather with colors that vary in some form along the x axis, we can achieve this effect by using either [`geom_ridgeline_gradient()`](https://wilkelab.org/ggridges/reference/geom_ridgeline_gradient.html) or [`geom_density_ridges_gradient()`](https://wilkelab.org/ggridges/reference/geom_ridgeline_gradient.html). Both geoms work just like `geom_ridgeline()` and `geom_density_ridges()`, except that they allow for varying fill colors. However, they do not allow for alpha transparency in the fill. For technical reasons, we can have changing fill colors or transparency but not both.
...
::: callout-tip
## One takeaway from the code chunk above:
...
## 3.3 Adding the Dot Plots with `stats_dots()`
...
Here I experiment with a few extended visualization ideas to explore the exam data beyond static plots.

## 4.1 Animating the Plot

First, I try animating a ridgeline density plot using `gganimate` to show how the English score distribution changes across classes and make comparisons more dynamic and engaging.
...
Besides, I explore stacked ridgelines by reshaping the data into a long format to allow English and Maths score distributions to be compared side by side within each class.
...
Finally, I experiment with a hybrid distribution plot that combines a half-violin (via `stat_halfeye`), boxplot, and jittered points to simultaneously display the overall distribution, summary statistics, and individual observations for English scores across classes.
